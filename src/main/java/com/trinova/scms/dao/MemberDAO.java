...
public class MemberDAO {
    ...
    public int register(Member m) throws SQLException {
        String sql = "INSERT INTO members (full_name, email, password_hash, role) " +
                     "VALUES (?, ?, ?, ?)";
    ...
    public void recordFailedAttempt(String email) throws SQLException {
        String sql = "UPDATE members SET failed_attempts = failed_attempts + 1, " +
    ...
    public void resetFailedAttempts(String email) throws SQLException {
        String sql = "UPDATE members SET failed_attempts = 0, is_locked = 0 WHERE email = ?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, email);
            ps.executeUpdate();
        }
    ...
    public void updateProfile(Member m) throws SQLException {
        String sql = "UPDATE members SET full_name = ?, phone = ?, bio = ?, " +
                     "profile_photo = ? WHERE member_id = ?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, m.getFullName());
    ...
    public void updatePassword(String email, String newHash) throws SQLException {
