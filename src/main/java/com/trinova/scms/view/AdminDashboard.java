...
public class AdminDashboard extends JFrame {
    ...
    private JPanel buildAdminHome() {
        ...
        try {
            com.trinova.scms.dao.MemberDAO memberDAO = new com.trinova.scms.dao.MemberDAO();
            com.trinova.scms.dao.RoomDAO roomDAO = new com.trinova.scms.dao.RoomDAO();
            com.trinova.scms.service.BookingService bookSvc = new com.trinova.scms.service.BookingService();

            int memberCount = memberDAO.getAllMembers().size();
            int roomCount = roomDAO.getAllRooms().size();
            int bookingCount = bookSvc.getAllBookings().size();
    ...
    private JPanel buildAllBookings() {
        ...
        refreshBtn.addActionListener(e -> {
            model.setRowCount(0);
            try {
                com.trinova.scms.service.BookingService svc = new com.trinova.scms.service.BookingService();
                for (com.trinova.scms.model.Booking b : svc.getAllBookings()) {
    ...
    private JPanel buildAllInvoices() {
        ...
        refreshBtn.addActionListener(e -> {
            model.setRowCount(0);
            try {
                com.trinova.scms.service.BillingService svc = new com.trinova.scms.service.BillingService();
    ...
    private JPanel buildSpaceManager() {
        ...
        refreshBtn.addActionListener(e -> {
            model.setRowCount(0);
            try {
                com.trinova.scms.dao.RoomDAO dao = new com.trinova.scms.dao.RoomDAO();
                for (Room r : dao.getAllRooms()) {
                    model.addRow(new Object[] {
                            ...
                            r.getRoomType(),
                            r.getCapacity(),
                            r.getPrivateSize() != null ? r.getPrivateSize() : "—",
                            r.getHourlyPrice() > 0 ? "PKR " + String.format(
                                    "%.0f",
                                    r.getHourlyPrice()) : "—",
                            r.getDailyPrice() > 0 ? "PKR " + String.format(
                                    "%.0f",
                                    r.getDailyPrice()) : "—",
                            r.getMonthlyPrice() > 0 ? "PKR " + String.format(
                                    "%.0f",
                                    r.getMonthlyPrice()) : "—",
                            r.isActive() ? "Yes" : "No"
                    });
        ...
        addBtn.addActionListener(e -> openRoomForm(null, panel, refreshBtn));
        ...
        editBtn.addActionListener(e -> {
            ...
            try {
                Room room = new com.trinova.scms.dao.RoomDAO()
                        .findById(roomId);
    ...
    private void openRoomForm(Room existing,
        ...
        JDialog dialog = new JDialog(this,
                existing == null ? "Add New Space" : "Edit Space",
                true);
        ...
        JTextField nameField = new JTextField(
                existing != null ? existing.getRoomName() : "", 20);
        ...
        JTextField amenitiesField = new JTextField(
                existing != null &&
                        existing.getAmenities() != null ? existing.getAmenities() : "",
                20);
        ...
        JTextField hourlyField = new JTextField(
                existing != null ? String.valueOf(
                        existing.getHourlyPrice()) : "0",
                10);
        JTextField dailyField = new JTextField(
                existing != null ? String.valueOf(
                        existing.getDailyPrice()) : "0",
                10);
        JTextField monthlyField = new JTextField(
                existing != null ? String.valueOf(
                        existing.getMonthlyPrice()) : "0",
                10);
    ...
    private JPanel buildPromoManager() {
        ...
        refreshBtn.addActionListener(e -> {
            model.setRowCount(0);
            try {
                com.trinova.scms.dao.SubscriptionDAO dao = new com.trinova.scms.dao.SubscriptionDAO();
                for (com.trinova.scms.model.PromoCode p : dao.getAllPromoCodes()) {
                    model.addRow(new Object[] {
                            p.getPromoId(),
                            p.getCode(),
                            p.getDiscountType(),
        ...
        addBtn.addActionListener(e -> openPromoForm(panel, refreshBtn));
        ...
        toggleBtn.addActionListener(e -> {
            ...
            boolean active = model.getValueAt(row, 4)
                    .equals("Yes");
            try {
                com.trinova.scms.dao.SubscriptionDAO dao = new com.trinova.scms.dao.SubscriptionDAO();
    ...
    private void openPromoForm(JPanel parent,
        ...
        saveBtn.addActionListener(e -> {
            try {
                ...
                com.trinova.scms.model.PromoCode p = new com.trinova.scms.model.PromoCode();
                p.setCode(code);
                p.setDiscountType(
                        (String) typeCombo
                                .getSelectedItem());
    ...
    private JPanel buildOccupancyReport() {
        ...
        refreshBtn.addActionListener(e -> {
            model.setRowCount(0);
            try {
                com.trinova.scms.dao.BookingDAO dao = new com.trinova.scms.dao.BookingDAO();
                List<com.trinova.scms.model.Booking> all = dao.getAllBookings();

                // Group by date
                java.util.Map<java.time.LocalDate, int[]> map = new java.util.TreeMap<>();

                for (com.trinova.scms.model.Booking b : all) {
                    if (b.getStatus().equals(
                            "CANCELLED"))
                        continue;
                    java.time.LocalDate d = b.getStartTime().toLocalDate();
        ...
        JComboBox<String> formatCombo = new JComboBox<>(
                new String[] { "PDF (.pdf)", "Word (.docx)",
                        "Text (.txt)" });
        formatCombo.setFont(new Font("Segoe UI",
                Font.PLAIN, 12));
        ...
        downloadBtn.setContentAreaFilled(true);
        downloadBtn.setBorderPainted(false);
        downloadBtn.setFont(new Font("Segoe UI",
                Font.BOLD, 12));
        ...
        downloadBtn.addActionListener(e -> {
            ...
            try {
                ...
                if (fmt.contains("PDF")) {
                    saved = com.trinova.scms.service.ReportExporter
                            .exportPdf(model, dir);
                } else if (fmt.contains("Word")) {
                    saved = com.trinova.scms.service.ReportExporter
                            .exportDocx(model, dir);
                } else {
                    saved = com.trinova.scms.service.ReportExporter
                            .exportTxt(model, dir);
                }
        ...
        // Right: Format combo + Download
        JPanel rightBtns = new JPanel(
                new FlowLayout(FlowLayout.RIGHT, 8, 0));
        rightBtns.setBackground(Color.WHITE);
        rightBtns.add(new JLabel("Format:"));
        rightBtns.add(formatCombo);
        rightBtns.add(downloadBtn);
    ...
    private JPanel buildMemberDirectory() {
        ...
        Runnable loadAll = () -> {
            model.setRowCount(0);
            try {
                com.trinova.scms.dao.MemberDAO dao = new com.trinova.scms.dao.MemberDAO();
                for (Member m : dao.getAllMembers()) {
                    model.addRow(new Object[] {
                            ...
                            m.getRole(),
                            m.getPlanType() != null ? m.getPlanType() : "None",
                            m.getPlanExpiry() != null ? m.getPlanExpiry() : "—",
                            m.getMemberId()
                    });
                }
            } catch (Exception ex) {
        ...
        lockBtn.addActionListener(e -> {
            ...
            try {
                ...
                if (m.isLocked()) {
                    ...
                    // to 5
                    java.sql.Connection conn = com.trinova.scms.dao.DatabaseConnection
                            .getInstance()
                            .getConnection();
